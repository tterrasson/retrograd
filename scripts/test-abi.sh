...
timed_step compile:ffi-lib cargo test --no-run -p retrograd-ffi --lib
timed_step compile:root-tests cargo test --no-run \
...
timed_step run:ffi-lib cargo test --lib -p retrograd-ffi
timed_step run:backend_devices cargo test --test backend_devices
timed_step run:metal_ops cargo test --test metal_ops
timed_step run:weighted_ce cargo test --test weighted_ce
...
timed_step run:gated_delta_net_chunked cargo test --test gated_delta_net_chunked
...
timed_step run:flash_attn_back cargo test --test flash_attn_back
...
timed_step run:out_prod_quant cargo test --test out_prod_quant -- --test-threads=1
timed_step run:engine_contracts cargo test --test engine_contracts \
...
timed_step run:rir_quant_oracle cargo test --test rir_quant_oracle
...
RETRO_RIR_MODE=off \
  timed_step run:rir_probe_off cargo test --test rir_probe
RETRO_RIR_MODE=prefer \
  timed_step run:rir_probe_prefer cargo test --test rir_probe
...
RETRO_RIR_MODE=require \
  timed_step run:rir_probe_require cargo test --test rir_probe
RETRO_RIR_MODE=require RETRO_RIR_TEST_REJECT=L2_NORM_BACK \
  timed_step run:rir_probe_require_reject cargo test --test rir_probe require_
...
RETRO_RIR_MODE=prefer RETRO_RIR_TEST_PREFER=CUMSUM \
  timed_step run:rir_probe_cumsum cargo test --test rir_probe cumsum
