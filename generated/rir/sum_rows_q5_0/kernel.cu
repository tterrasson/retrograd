...
// Kernel: sum_rows_q5_0 (backend cuda, reduction subgroup_tree, scan serial, variante repli).
...
static void rir_k_sum_rows_q5_0(
        const uint8_t * __restrict__ x,
        uint8_t * __restrict__ y,
        const rir_sum_rows_q5_0_params p)
{
    ...
    for (uint32_t col_seg_v4 = seg_start_v3; col_seg_v4 < p.n_col; col_seg_v4 += 128u) {
        ...
        const uint32_t base_v29 = blk_v5 * p.x_nb0 + row_v0 * p.x_nb1 + 6u;
        const uint32_t base_v30 = blk_v5 * p.x_nb0 + row_v0 * p.x_nb1 + 2u;
        for (uint32_t col_e_v8 = 0u; col_e_v8 < 4u; ++col_e_v8) {
            const uint32_t inb_v10 = inb0_v6 + col_e_v8;
            const uint32_t q_inp_v11 = inb_v10 % 16u;
            const uint32_t q_byte_v12 = uint32_t(*(const uint8_t *)(x + (q_inp_v11 + base_v29)));
            ...
            const uint32_t qh_grp_v17 = inb_v10 / 8u;
            const uint32_t qh_byte_v18 = uint32_t(*(const uint8_t *)(x + (qh_grp_v17 + base_v30)));
            const uint32_t qh_ing_v19 = inb_v10 % 8u;
            const uint32_t qh_shr_v20 = qh_byte_v18 >> qh_ing_v19;
            const uint32_t qh_v21 = qh_shr_v20 & 1u;
            const uint32_t qh_up_v22 = qh_v21 * 16u;
            const uint32_t q_full_v23 = q_v16 | qh_up_v22;
...
extern "C" void rir_launch_sum_rows_q5_0(
    ...
    rir_sum_rows_q5_0_params p;
    std::memcpy(&p, params, sizeof p);
    rir_k_sum_rows_q5_0<<<dim3(grid[0], grid[1], grid[2]), dim3(32, 1, 1), 0, stream>>>(
