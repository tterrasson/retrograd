...
// Kernel: scale (backend cuda, reduction serial, scan serial, variante repli).
...
static void rir_k_scale(
        const uint8_t * __restrict__ a,
        uint8_t * __restrict__ dst,
        const rir_scale_params p)
{
    const uint32_t flat_v15 = blockIdx.x * blockDim.x + threadIdx.x;
    if (flat_v15 >= p.rir_flat_total) {
        return;
    }
    const uint32_t col_v0 = (rir_fastmod(flat_v15, p.rir_flat0_mp, p.rir_flat0_sh, p.rir_flat0_div)) * 4u;
    const uint32_t flat_v15_q1 = rir_fastdiv(flat_v15, p.rir_flat0_mp, p.rir_flat0_sh);
    const uint32_t row_v1 = rir_fastmod(flat_v15_q1, p.rir_flat1_mp, p.rir_flat1_sh, p.rir_flat1_div);
    const uint32_t flat_v15_q2 = rir_fastdiv(flat_v15_q1, p.rir_flat1_mp, p.rir_flat1_sh);
    const uint32_t plane_v2 = rir_fastmod(flat_v15_q2, p.rir_flat2_mp, p.rir_flat2_sh, p.rir_flat2_div);
    const uint32_t flat_v15_q3 = rir_fastdiv(flat_v15_q2, p.rir_flat2_mp, p.rir_flat2_sh);
    const uint32_t batch_v3 = flat_v15_q3;
    const uint32_t base_v16 = row_v1 * p.a_nb1 + plane_v2 * p.a_nb2 + batch_v3 * p.a_nb3;
    const float p_v11 = p.scale;
    const float p_v13 = p.bias;
    const uint32_t base_v17 = row_v1 * p.dst_nb1 + plane_v2 * p.dst_nb2 + batch_v3 * p.dst_nb3;
    if (col_v0 + 4u <= p.n_col) {
        ...
        const rir_vf<4> t_v6 = t_v4 * p_v11;
        const rir_vf<4> t_v8 = t_v6 + p_v13;
        const uint32_t t_v8_o = col_v0 * p.dst_nb0 + row_v1 * p.dst_nb1 + plane_v2 * p.dst_nb2 + batch_v3 * p.dst_nb3;
        *(float *)(dst + (t_v8_o)) = t_v8.c[0];
        *(float *)(dst + (t_v8_o + 4u)) = t_v8.c[1];
        *(float *)(dst + (t_v8_o + 8u)) = t_v8.c[2];
        *(float *)(dst + (t_v8_o + 12u)) = t_v8.c[3];
    } else {
        for (uint32_t col_tail_v9 = col_v0; col_tail_v9 < p.n_col; ++col_tail_v9) {
            const float t_v10 = *(const float *)(a + (col_tail_v9 * p.a_nb0 + base_v16));
            const float t_v12 = t_v10 * p_v11;
            const float t_v14 = t_v12 + p_v13;
            *(float *)(dst + (col_tail_v9 * p.dst_nb0 + base_v17)) = t_v14;
        }
    }
}

extern "C" void rir_launch_scale(
    ...
    rir_k_scale<<<dim3(grid[0], grid[1], grid[2]), dim3(256, 1, 1), 0, stream>>>(
