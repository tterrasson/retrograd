...
// Kernel: add (backend cuda, reduction serial, scan serial, variante flat_linear).
...
static void rir_k_add_flat_linear(
        const uint8_t * __restrict__ a,
        const uint8_t * __restrict__ b,
        uint8_t * __restrict__ dst,
        const rir_add_flat_linear_params p)
{
    ...
    const uint32_t t_v6_i = flat_v4 * 16u;
    const rir_vf<4> t_v6 = {{*(const float *)(b + (t_v6_i)), *(const float *)(b + (t_v6_i + 4u)), *(const float *)(b + (t_v6_i + 8u)), *(const float *)(b + (t_v6_i + 12u))}};
    const rir_vf<4> t_v7 = t_v5 + t_v6;
    const uint32_t t_v7_o = flat_v4 * 16u;
    *(float *)(dst + (t_v7_o)) = t_v7.c[0];
    *(float *)(dst + (t_v7_o + 4u)) = t_v7.c[1];
    *(float *)(dst + (t_v7_o + 8u)) = t_v7.c[2];
    *(float *)(dst + (t_v7_o + 12u)) = t_v7.c[3];
}

extern "C" void rir_launch_add_flat_linear(
    ...
    rir_add_flat_linear_params p;
    std::memcpy(&p, params, sizeof p);
    rir_k_add_flat_linear<<<dim3(grid[0], grid[1], grid[2]), dim3(256, 1, 1), 0, stream>>>(
