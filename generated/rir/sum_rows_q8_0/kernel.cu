...
// Kernel: sum_rows_q8_0 (backend cuda, reduction subgroup_tree, scan serial, variante repli).
...
static void rir_k_sum_rows_q8_0(
        const uint8_t * __restrict__ x,
        uint8_t * __restrict__ y,
        const rir_sum_rows_q8_0_params p)
{
    ...
    const float red4_v13 = rir_lane_sum(acc4_v2);
    if (lane_v1 == 0u) {
        *(float *)(y + (row_v0 * p.y_nb0)) = red4_v13;
    }
}

extern "C" void rir_launch_sum_rows_q8_0(
    ...
    rir_sum_rows_q8_0_params p;
    std::memcpy(&p, params, sizeof p);
    rir_k_sum_rows_q8_0<<<dim3(grid[0], grid[1], grid[2]), dim3(32, 1, 1), 0, stream>>>(
