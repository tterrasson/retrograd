        ...
        const rir_sum_rows_q5_1_params p)
{
    ...
    const uint32_t base_v30 = row_v0 * p.x_nb1;
    for (uint32_t col_seg_v4 = seg_start_v3; col_seg_v4 < p.n_col; col_seg_v4 += 128u) {
        ...
        const float min_v8 = __half2float(*(const __half *)(x + (blk_v5 * p.x_nb0 + base_v30 + 2u)));
        ...
        for (uint32_t col_e_v9 = 0u; col_e_v9 < 4u; ++col_e_v9) {
            ...
            const uint32_t q_byte_v13 = uint32_t(*(const uint8_t *)(x + (q_inp_v12 + base_v29 + 8u)));
            ...
            const uint32_t qh_byte_v19 = uint32_t(*(const uint8_t *)(x + (qh_grp_v18 + base_v29 + 4u)));
