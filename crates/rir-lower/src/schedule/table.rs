...
pub fn schedules_for(family: Family) -> Vec<Schedule> {
    ...
    match family {
        ...
        Family::Unary => unary(&mut v),
        Family::ElementwiseRepeat => elementwise_repeat(&mut v),
        Family::Elementwise => elementwise(&mut v),
        Family::Scale => scale(&mut v),
        Family::MatMulNaive => mat_mul_naive(&mut v),
        Family::OutProd => out_prod(&mut v),
        Family::Cumsum => cumsum(&mut v),
    }
    v
}
...
fn row_reduce(v: &mut Vec<Schedule>) {
    let family = Family::RowReduce;
...
fn rms_norm(v: &mut Vec<Schedule>) {
    let family = Family::RmsNorm;
    for gpu in targets_for(family) {
        v.push(Schedule::gpu_subgroup(gpu));
    }
    let few_wide_rows = || {
...
fn unary(v: &mut Vec<Schedule>) {
    let family = Family::Unary;
    ...
    // an address at once.
    for gpu in flat_targets_for(family) {
        v.push(Schedule::gpu_grid_flat_linear(gpu, [256, 1, 1], 1).claiming(90, Vec::new()));
    }
}

fn elementwise_repeat(v: &mut Vec<Schedule>) {
    let family = Family::ElementwiseRepeat;
    for gpu in grid_targets_for(family) {
...
fn elementwise(v: &mut Vec<Schedule>) {
    ...
    for gpu in flat_targets_for(family) {
        v.push(Schedule::gpu_grid_flat_linear(gpu, [256, 1, 1], 4).claiming(100, Vec::new()));
    }
}
...
/// generated kernel.
fn scale(v: &mut Vec<Schedule>) {
    let family = Family::Scale;
    for gpu in grid_targets_for(family) {
        v.push(Schedule::gpu_grid_vec4(gpu, [256, 1, 1]).unnamed());
    }
    // The flattened dispatch, on the backends
    ...
    // is something else.
    for gpu in flat_targets_for(family) {
        v.push(Schedule::gpu_grid_flat_linear(gpu, [256, 1, 1], 4).claiming(90, Vec::new()));
    }
}

/// Two parallel axes (i, j): no collective, one invocation per output
/// point, and a sequential contraction within the invocation.
fn mat_mul_naive(v: &mut Vec<Schedule>) {
    let family = Family::MatMulNaive;
    for gpu in targets_for(family) {
...
fn out_prod(v: &mut Vec<Schedule>) {
    let family = Family::OutProd;
    for gpu in targets_for(family) {
