//! The two C headers the registry ships: the fixed contract header, and the
//! per-kernel parameter structs derived from `shader_params_layout`.

use rir_lower::LoopKernel;

use crate::integration::IntegrationSpec;
use crate::manifest::{artifact_name, shader_params_layout};

use super::*;

pub const REGISTRY_HEADER: &str = r#"// Generated by rir-gen - DO NOT EDIT MANUALLY.
...
pub(crate) fn max_rank(k: &LoopKernel) -> usize {
    // The published contract: the tightest declared Rank constraint, or the
    // widest argument rank when no constraint narrows it.
    let declared = k
        .constraints
        .iter()
        .filter_map(|c| match c {
            rir_core::Constraint::Rank { max, .. } => Some(*max),
            _ => None,
        })
        .min();
    declared.unwrap_or_else(|| k.args.iter().map(|a| a.ty.rank).max().unwrap_or(0))
}

/// Emits `rir_kernel_params.h`: one host-side constant-buffer struct per
/// **CUDA artifact**, with the `static_assert` that pins its size.
///
/// Per artifact and not per kernel, and the change is
/// forced rather than tidy: the flattened dispatch publishes its divisors in
/// the constant buffer, so `add` and `add_flat` are one kernel with two
/// layouts. Keying on the kernel made that pair "two constant layouts" - a
/// refusal that was right about the header and wrong about the world. A
/// fallback's artifact name *is* its kernel name, so nothing that existed
/// before is renamed by this.
///
/// This is the layout `shader_params_layout` already gives GLSL and MSL, in a
/// third vocabulary. Emitting it removes the last place where a backend
/// restated it by hand - the two 68-byte structs Metal and Vulkan each declared
/// were a silent ABI contract between generated code and code nothing checked.
///
/// `oracle` is the second argument CUDA made necessary.
/// A `.comp` and a `.metal` declare their own params struct; a `.cu` does not:
/// its whole gain is that it includes *this* header instead of restating the
/// ABI. So a kernel with a CUDA artifact needs a struct here whether or not any
/// backend dispatches it, and the twelve quantized reductions and the two
/// autodiff bench kernels are exactly that case: oracle-only kernels the parity
/// harness compiles and no dispatcher can reach. Emitting a second layout for
/// them would be the duplication this header exists to remove; leaving them out
/// makes their `.cu` fail to compile, which is how the gap was found.
///
/// They land after the production ones and change nothing about them,
/// including `RIR_MAX_PUSH_CONSTANT_BYTES`, which stays a bound over production
/// alone.
///
/// # Errors
///
/// Returns `RegistryError` if two production backends for the same kernel
/// publish two constant layouts: one host struct cannot serve both.
pub fn emit_params_header(
    variants: &[(&LoopKernel, &IntegrationSpec)],
    oracle: &[&LoopKernel],
) -> Result<String, RegistryError> {
    let mut out = String::new();
    out.push_str("// Generated by rir-gen - DO NOT EDIT MANUALLY.\n");
    out.push_str("// Host-side constant-buffer layouts of the RIR kernels.\n");
    out.push_str("// Same layout as the generated shader's own\n");
    out.push_str("// params struct: kernel parameters, then axis extents, then one byte\n");
    out.push_str("// stride per argument dimension, four bytes each.\n");
    out.push_str("#pragma once\n");
    out.push_str("#include <stdint.h>\n\n");

    let mut emitted: Vec<(String, Vec<(String, &'static str)>)> = Vec::new();
    for k in variants
        .iter()
        .map(|(k, _)| *k)
        .chain(oracle.iter().copied())
    {
        // CUDA lowerings only, and that is not a narrowing: a `.comp`
        // and a `.metal` declare their own params struct; a `.cu` is the one
        // artifact that includes this header.
        // Emitting a struct for the Vulkan lowering of an artifact whose CUDA
        // lowering has a different layout - a flattened one, which publishes
        // its divisors - was the only way two backends could disagree here, and
        // they disagreed about a struct neither of them reads.
        if k.schedule.backend() != rir_lower::Backend::Cuda {
            continue;
        }
        let layout = shader_params_layout(k);
        let name = artifact_name(k);
        if let Some((_, previous)) = emitted.iter().find(|(n, _)| *n == name) {
            // Two backends running the same artifact must publish the same
            // constant buffer, or one host struct cannot serve both.
            if previous != &layout {
                return Err(RegistryError::new(
                    &k.name,
                    "two production backends, two constant layouts",
                ));
            }
            continue;
        }
        out.push_str(&format!("typedef struct rir_{name}_params {{\n"));
        for (pname, ty) in &layout {
            let c_ty = match *ty {
                "float" => "float   ",
                "int" => "int32_t ",
                _ => "uint32_t",
            };
            out.push_str(&format!("    {c_ty} {pname};\n"));
        }
        out.push_str(&format!("}} rir_{name}_params;\n"));
        out.push_str(&format!(
            "static_assert(sizeof(rir_{name}_params) == {}, \"registry: push_constant_bytes\");\n\n",
            4 * layout.len()
        ));
        emitted.push((name, layout));
    }

    // A dispatcher that does not know the kernel still needs somewhere to build
    // the constant buffer. Publishing the bound lets it stay on the stack
    // instead of allocating once per node.
    // Over the **production** kernels only, which is what the sentence above
    // says and what a dispatcher sizes a stack buffer against. An oracle kernel
    // has a struct here (see the doc comment) but reaches no dispatcher, so
    // letting it raise this bound would grow every adapter's buffer for a
    // kernel none of them can encode.
    let max_bytes = variants
        .iter()
        .map(|(k, _)| 4 * shader_params_layout(k).len())
        .max()
        .unwrap_or(0);
    out.push_str(
    ...
    out.push_str(&format!(
        "#define RIR_MAX_PUSH_CONSTANT_BYTES {max_bytes}\n"
    ));
    Ok(out)
}
